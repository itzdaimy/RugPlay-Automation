...
async function sendBet(amount, cookie) {
  try {
    const res = await axios.post(API_URL, { amount }, {
...
function makeEmbed(res, actualSide, bet, winStreak, lossStreak, wins, losses, balanceCurrent, balanceStart, attempts, maxAttempts, config) {
  ...
  return new EmbedBuilder()
    ...
    .addFields(
      ...
      { name: "Symbols", value: res.symbols.join(" | "), inline: true },
...
async function runBot() {
  ...
  while (running) {
    ...
    console.log(chalk.cyan(`\n🎰 Attempt ${attempts + 1}: Spinning slots with ${bet.toFixed(2)}`));

    const res = await sendBet(bet, cookie);
    if (!res) {
    ...
    lossStreak = 0;
    winStreak++;
    console.log(chalk.green(`✅ WIN! Symbols: [${res.symbols.join(', ')}] | Payout: ${res.payout.toFixed(2)}`));
    } else {
    losses++;
    lossStreak++;
    winStreak = 0;
    totalLoss += bet;
    console.log(chalk.red(`❌ LOSS! Symbols: [${res.symbols.join(', ')}]`));
