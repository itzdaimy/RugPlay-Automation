...
const API_URL = "https://rugplay.com/api/gambling/slots";
        ...
        switch (choice) {
            case 'basic':
                const basicResponses = await prompts([
                    ...
                    { type: 'number', name: 'delayBetween', message: 'Delay between spins (ms)', initial: config.delayBetween || 1500 }
                ]);
...
function viewStatistics() {
    ...
    console.log(`Total Spins: ${chalk.yellow(history.length)}`);
...
async function sendBet(amount, cookie) {
  try {
    const res = await axios.post(API_URL, { amount }, { headers: { Cookie: cookie, 'Content-Type': 'application/json' }});
    return res.data;
  } catch (err) {
...
function makeEmbed(data) {
    const { res, bet, stats, balance } = data;
    ...
    return new EmbedBuilder()
        ...
        .setDescription(`Symbols: **${res.symbols.join(' | ')}**`)
        .addFields(
...
async function runBot() {
    ...
    console.log(chalk.cyanBright("🚀 Starting Slots Bot... Press CTRL+C to stop.\n"));
    ...
    while (running) {
        ...
        console.log(chalk.cyan(`\n🎰 Spinning with ${chalk.yellow(betAmount.toFixed(2))}`));

        const res = await sendBet(betAmount, config.cookie);
        ...
        if (res.won) {
            ...
            stats.lossStreak = 0;
            stats.winStreak++;
            console.log(chalk.green(`✔ WIN! [${res.symbols.join(', ')}] | Payout: ${res.payout.toFixed(2)}`));
        } else {
            stats.losses++;
            stats.winStreak = 0;
            stats.lossStreak++;
            console.log(chalk.red(`✘ LOSS! [${res.symbols.join(', ')}]`));
        }

        currentBet = calculateNextBet(currentBet, initialBet, maxBet, stats.winStreak, stats.lossStreak, bettingStrategy);
        const profit = balance.current - balance.start;
        const historyEntry = { timestamp: new Date().toISOString(), result: res.won ? 'win' : 'loss', amount: betAmount, payout: res.payout, balance: +balance.current.toFixed(2), profit: +profit.toFixed(2) };
        ...
        if (webhookClient) {
            const embed = makeEmbed({ res, bet: betAmount, stats, balance });
